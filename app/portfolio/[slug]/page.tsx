...
export default async function ProjectDetails({
  ...
  if (projectData.data[0].tags && projectData.data[0].tags.data.length) {
    ...
    const filteredCaseStudies = allCaseStudies.filter(
      (cs: any) => cs.id !== projectData.data[0].id
    );
    relatedCaseStudies = Array.from(
      new Map(filteredCaseStudies.map((cs: any) => [cs.id, cs])).values()
    );
  }
  ...
  const imageServerPrefix =
    "https://informed-captain-64ef5bbe8f.media.strapiapp.com";
  ...
  return (
    <main className="flex flex-col items-center h-content w-screen my-32">
      <div className="relative t-0 flex flex-col w-full h-[350px] sm:h-[600px] sm:w-3/4 ">
        {mediaUrl && (
          <Image
            src={media.data[0].url}
            alt={media.data[0].alternativeText}
            width={1280}
            height={1000}
      ...
      <Card className="w-3/4 bg-transparent border-transparent text-white">
        <CardHeader>
          ...
          <CardDescription className="text-fronzBlue text-2xl">
            {client}
          </CardDescription>
        </CardHeader>
        <div className="p-8">
      ...
      <div className="h-content w-screen xl:w-3/4 items-center justify-center py-8 mb-4 mx-auto">
        {projectData.data[0].videoGallery.map((v: any) => {
          const providerUid = v.video?.providerUid;
          const videoTitle = v.video?.videoTitle;

          if (!providerUid) {
            console.error("providerUid is undefined for video:", v);
            return null; // Skip rendering this iframe
          }
        ...
        {media.data.length > 1 && (
          <>
            <h3 className="text-fronzGold text-3xl text-center my-2">
              Image Gallery
            </h3>
